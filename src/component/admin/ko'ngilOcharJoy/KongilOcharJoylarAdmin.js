...
function KongilOcharJoylarAdmin() {
    ...
    const getHotelsAdmin = () => {
        axios.get(apiTravel + "places/?category=8").then(res => setAdminHotel(res.data.results));
    }

    // add
    const addRestoran = () => {
        const kongilOcharPlaces = new FormData();
        ...
        kongilOcharPlaces.append("image", byIdImg('image'));
        if (image2 != null) kongilOcharPlaces.append("image2", byIdImg('image2'));
        if (image3 != null) kongilOcharPlaces.append("image3", byIdImg('image3'));
        if (image4 != null) kongilOcharPlaces.append("image4", byIdImg('image4'));
        if (image5 != null) kongilOcharPlaces.append("image5", byIdImg('image5'));
        if (image6 != null) kongilOcharPlaces.append("image6", byIdImg('image6'));
        if (image7 != null) kongilOcharPlaces.append("image7", byIdImg('image7'));
        if (image8 != null) kongilOcharPlaces.append("image8", byIdImg('image8'));
        kongilOcharPlaces.append('title', byId('title_uz'));
        kongilOcharPlaces.append('title_uz', byId('title_uz'));
        kongilOcharPlaces.append('title_en', byId('title_en'));
        kongilOcharPlaces.append('title_ru', byId('title_ru'));
        kongilOcharPlaces.append('description', byId('description_uz'));
        kongilOcharPlaces.append('description_uz', byId('description_uz'));
        kongilOcharPlaces.append('description_en', byId('description_en'));
        kongilOcharPlaces.append('description_ru', byId('description_ru'));
        kongilOcharPlaces.append('description2', byId('description2_uz'));
        kongilOcharPlaces.append('description2_uz', byId('description2_uz'));
        kongilOcharPlaces.append('description2_en', byId('description2_en'));
        kongilOcharPlaces.append('description2_ru', byId('description2_ru'));
        kongilOcharPlaces.append('description3', byId('description3_uz'));
        kongilOcharPlaces.append('description3_uz', byId('description3_uz'));
        kongilOcharPlaces.append('description3_en', byId('description3_en'));
        kongilOcharPlaces.append('description3_ru', byId('description3_ru'));
        kongilOcharPlaces.append('rank', byId('rank'));
        kongilOcharPlaces.append('booking_link', byId('booking_link'));
        kongilOcharPlaces.append('category', 8);
        ...
        axios.post(apiTravel + "places/", kongilOcharPlaces, config).then(() => {
            openAddModal();
            getHotelsAdmin();
            toast.success("Ko'ngil ochar joylar muvaffaqiyatli qo'shildi✔");
        }).catch(() => {
    ...
    const editRestoran = () => {
        ...
        axios.put(apiTravel + "places/" + adminHotelId.id + "/", kongilOcharPlaces, config).then(() => {
            openEditModal();
            getHotelsAdmin();
            toast.success("Ko'ngil ochar joylar muvaffaqiyatli taxrirlandi✔");
        }).catch(() => {
            toast.error("So'rovda xatolik yuz berdi! Ma'lumotlarni qaytadan tekshirib ko'ring!!!");
            openEditModal();
        })
    ...
    const deleteRestoran = () => {
        ...
        axios.delete(apiTravel + "places/" + adminHotelId.id + "/", config).then(() => {
            openDeleteModal();
            getHotelsAdmin();
            toast.success("Ko'ngil ochar joy muvaffaqiyatli o'chirildi")
        }).catch(() => {
    ...
    function searchVal() {
        ...
        if (!!searchVal) axios.get(apiTravel + 'places/?category=8&search=' + searchVal).then(res => setAdminHotel(res.data.results))
        else getHotelsAdmin();
    }
    ...
    return (
        <Container>
            ...
            <h1
                ...
                className="text-center fw-bold text-dark">Ko'ngil ochar joylar Admin</h1>

            <div className="mb-5"
                style={{ display: "flex", justifyContent: "space-between", flexDirection: "row" }}>
                <Button onClick={openAddModal} color="primary" className="px-5 py-2 fs-5 fw-medium">Ko'ngil ochar joylar qo'shish</Button>
                <div>
                    <InputGroup>
            ...
            <Modal size="xl" isOpen={addModal} scrollable>
                <ModalHeader toggle={openAddModal}><h3>Ko'ngil ochar joylar qo'shish</h3></ModalHeader>
            ...
            <Modal size="xl" scrollable isOpen={editModal}>
                <ModalHeader toggle={openEditModal}><h3>Ko'ngil ochar joylarni tahrirlash</h3></ModalHeader>
                <ModalBody className="modal__label">
                    <Row className="w-100">
                        <Col className="col-12 col-md-4">
                            <Label for="image">Image: Majburiy</Label>
            ...
            <Modal size="lg" isOpen={deleteModal} scrollable>
                <ModalHeader toggle={openDeleteModal}><h3>Ko'ngil ochar joylarni o'chirish</h3></ModalHeader>
...
export default KongilOcharJoylarAdmin;